...
if [ "${V7_ENV_READY:-0}" != "1" ]; then
  echo "[run_v7_stage2_resolve_structures.sh][ERROR] V7 Python environment is not ready. Run: bash deploy/env_v7.sh" >&2
  exit 2
fi
...
cmd=("$PYTHON_BIN" "$V7_DIR/tools/resolve_structures_v62.py"
  ...
  --retry-backoff-sec "${V7_COLABFOLD_RETRY_BACKOFF_SEC:-60}"
