#!/usr/bin/env python3
"""Traceable SynBio GFP V7 Orthogonal-Validation runner.

Each stage can be run independently, while this orchestrator provides a one-click path.
Structure handling is delegated to the V7 resolver so reusable PDBs are consumed before
...
def now_id() -> str:
    return time.strftime("v7_%Y%m%d_%H%M%S")
...
def main() -> None:
    p = argparse.ArgumentParser(description="Run SynBio GFP V7 Adaptive pipeline.")
    ...
    p.add_argument("--cache-dir", default=os.environ.get("V7_CACHE_DIR", str(ROOT / "cache" / "v7")))
    p.add_argument("--output-root", default=os.environ.get("V7_OUTPUT_ROOT", str(ROOT / "outputs")))
    ...
    p.add_argument("--reference-pdb", default=os.environ.get("V7_REFERENCE_PDB", os.environ.get("REF_PDB", str(ROOT / "reference" / "reference_gfp.pdb"))))
    ...
    p.add_argument("--structure-metrics-csv", default=os.environ.get("V7_STRUCTURE_METRICS_CSV", ""))
    p.add_argument("--proteinmpnn-score-csv", default=os.environ.get("V7_PROTEINMPNN_SCORE_CSV", ""))
    p.add_argument("--softcheck-csv", default=os.environ.get("V7_SOFTCHECK_CSV", ""))
    p.add_argument("--run-softcheck", action="store_true", default=os.environ.get("RUN_V7_SOFTCHECK", "0") == "1")
    p.add_argument("--softcheck-runner", choices=["import", "metrics-only", "external"], default=os.environ.get("V7_SOFTCHECK_RUNNER", "metrics-only"))
    p.add_argument("--softcheck-pdb-dir", default=os.environ.get("V7_SOFTCHECK_PDB_DIR", ""))
    p.add_argument("--softcheck-command", default=os.environ.get("OPENCOMPLEX2_COMMAND", os.environ.get("V7_SOFTCHECK_COMMAND", "")))
    p.add_argument("--softcheck-top-n", type=int, default=int(os.environ.get("V7_SOFTCHECK_TOP_N", "80")))
    p.add_argument("--softcheck-timeout-sec", type=int, default=int(os.environ.get("V7_SOFTCHECK_TIMEOUT_SEC", "21600")))
    ...
    p.add_argument("--colabfold-msa-mode", default=os.environ.get("V7_COLABFOLD_MSA_MODE", "mmseqs2_uniref_env"))
    p.add_argument("--structure-chunk-size", type=int, default=int(os.environ.get("STRUCTURE_CHUNK_SIZE", "12")))
    p.add_argument("--chunk-timeout-sec", type=int, default=int(os.environ.get("V7_COLABFOLD_CHUNK_TIMEOUT_SEC", "7200")))
    p.add_argument("--chunk-retries", type=int, default=int(os.environ.get("V7_COLABFOLD_CHUNK_RETRIES", "1")))
    ...
    p.add_argument("--previous-ranked-csv", default=os.environ.get("V7_PREVIOUS_RANKED_CSV", os.environ.get("V6_PREVIOUS_RANKED_CSV", "")))
    p.add_argument("--feedback-csv", default=os.environ.get("V7_FEEDBACK_CSV", os.environ.get("V6_FEEDBACK_CSV", "")))
    p.add_argument("--brightness-teacher-csv", default=os.environ.get("V7_BRIGHTNESS_TEACHER_CSV", ""))
    ...
    dirs = {
        ...
        "softcheck": run_dir / "04_softcheck",
        "final": run_dir / "05_final",
    ...
    context.update({"v7_dir": str(ROOT), "run_dir": str(run_dir), "python": sys.executable})
    (run_dir / "v7_run_context.json").write_text(json.dumps(context, indent=2, ensure_ascii=False), encoding="utf-8")
    status_rows: list[dict[str, Any]] = []
    status_csv = dirs["lineage"] / "v7_stage_status.csv"
    commands_sh = dirs["lineage"] / "v7_commands.sh"
    ...
    try:
        ...
        softcheck_scores = dirs["softcheck"] / "softcheck_metrics_v7.csv"
        if args.softcheck_csv:
            shutil.copy2(args.softcheck_csv, softcheck_scores)
            run_stage("04_softcheck_import", ["bash", "-lc", f"wc -l {shlex.quote(str(softcheck_scores))}"], dirs["logs"] / "04_softcheck.log", status_rows, args.dry_run)
        elif args.run_softcheck:
            soft_cmd = [
                sys.executable, str(ROOT / "tools" / "run_opencomplex2_softcheck_v7.py"),
                "--candidates-csv", str(priority_csv),
                "--reference-pdb", args.reference_pdb,
                "--out-csv", str(softcheck_scores),
                "--work-dir", str(dirs["softcheck"] / "softcheck_work"),
                "--runner", args.softcheck_runner,
                "--top-n", str(args.softcheck_top_n),
                "--timeout-sec", str(args.softcheck_timeout_sec),
            ]
            if args.softcheck_pdb_dir:
                soft_cmd += ["--pdb-dir", args.softcheck_pdb_dir]
            if args.softcheck_command:
                soft_cmd += ["--external-command-template", args.softcheck_command]
            command_lines.append(shell_quote(soft_cmd))
            run_stage("04_opencomplex2_softcheck", soft_cmd, dirs["logs"] / "04_softcheck.log", status_rows, args.dry_run)
        write_status(status_rows, status_csv)
        ...
        final_cmd = [
            sys.executable, str(ROOT / "scripts" / "run_v7_final_from_manifest.py"),
            "--config", args.config,
        ...
        if softcheck_scores.exists() or args.dry_run:
            final_cmd += ["--softcheck-csv", str(softcheck_scores)]
        command_lines.append(shell_quote(final_cmd))
        run_stage("05_final_scoring_portfolio", final_cmd, dirs["logs"] / "05_final.log", status_rows, args.dry_run)
        write_status(status_rows, status_csv)

        audit_cmd = [
            sys.executable, str(ROOT / "tools" / "audit_submission_v7.py"),
            "--submission-csv", str(dirs["final"] / "submission_v7_for_upload.csv"),
            "--final-csv", str(dirs["final"] / "final_top6_v7.csv"),
            "--data-dir", args.data_dir,
            "--config", args.config,
            "--out-json", str(dirs["final"] / "v7_submission_audit.json"),
            "--out-md", str(dirs["final"] / "v7_submission_audit.md"),
            "--fail-on-error",
        ]
        command_lines.append(shell_quote(audit_cmd))
        run_stage("06_submission_audit", audit_cmd, dirs["logs"] / "06_audit.log", status_rows, args.dry_run)
        write_status(status_rows, status_csv)
        ...
        run_stage("07_lineage_index", lineage_cmd, dirs["logs"] / "07_lineage.log", status_rows, args.dry_run)
        write_status(status_rows, status_csv)
    finally:
    ...
    print("V7 evidence-safe run completed")
    print(f"run_dir: {run_dir}")
    print(f"stage_status: {status_csv}")
    print(f"commands: {commands_sh}")
    print(f"submission: {dirs['final'] / 'submission_v7_for_upload.csv'}")
