...
def now_id() -> str:
    return time.strftime("v61_%Y%m%d_%H%M%S")
...
def main() -> None:
    ...
    p.add_argument("--config", default=str(ROOT / "configs" / "v61_adaptive.yaml"))
    p.add_argument("--cache-dir", default=os.environ.get("V61_CACHE_DIR", str(ROOT / "cache" / "v61")))
    p.add_argument("--output-root", default=os.environ.get("V61_OUTPUT_ROOT", str(ROOT / "outputs")))
    ...
    p.add_argument("--reference-pdb", default=os.environ.get("V61_REFERENCE_PDB", os.environ.get("REF_PDB", str(ROOT / "reference" / "reference_gfp.pdb"))))
    ...
    p.add_argument("--structure-metrics-csv", default=os.environ.get("V61_STRUCTURE_METRICS_CSV", ""))
    p.add_argument("--proteinmpnn-score-csv", default=os.environ.get("V61_PROTEINMPNN_SCORE_CSV", ""))
    ...
    p.add_argument("--previous-ranked-csv", default=os.environ.get("V61_PREVIOUS_RANKED_CSV", os.environ.get("V6_PREVIOUS_RANKED_CSV", "")))
    p.add_argument("--feedback-csv", default=os.environ.get("V61_FEEDBACK_CSV", os.environ.get("V6_FEEDBACK_CSV", "")))
    p.add_argument("--brightness-teacher-csv", default=os.environ.get("V61_BRIGHTNESS_TEACHER_CSV", ""))
    ...
    context.update({"v61_dir": str(ROOT), "run_dir": str(run_dir), "python": sys.executable})
    (run_dir / "v61_run_context.json").write_text(json.dumps(context, indent=2, ensure_ascii=False), encoding="utf-8")
    status_rows: list[dict[str, Any]] = []
    status_csv = dirs["lineage"] / "v61_stage_status.csv"
    commands_sh = dirs["lineage"] / "v61_commands.sh"
    ...
    try:
        ...
        stage1_cmd = [
            sys.executable, str(ROOT / "scripts" / "run_v61_complete.py"),
            "--config", args.config,
        ...
        if args.structure_metrics_csv:
            ...
            resolver_cmd = [
                sys.executable, str(ROOT / "tools" / "resolve_structures_v61.py"),
        ...
        if args.brightness_teacher_csv:
            final_cmd += ["--brightness-teacher-csv", args.brightness_teacher_csv]
    ...
    print("V6.1 adaptive run completed")
    print(f"run_dir: {run_dir}")
    print(f"stage_status: {status_csv}")
    print(f"commands: {commands_sh}")
    print(f"submission: {dirs['final'] / 'submission_v61.csv'}")
