...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run complete SynBio GFP V6.1 Adaptive traceable pipeline")
...
def resolve_v6_config(raw: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    ...
    if getattr(args, "brightness_teacher_csv", None):
        cfg.setdefault("v61", {}).setdefault("brightness_teacher", {})["previous_ranked_csv"] = args.brightness_teacher_csv
...
def infer_v6_bucket(row: pd.Series) -> str:
    ...
    if source.startswith("feedback") or feedback_score >= 0.68:
        return "feedback_guided_exploitation"
    ...
    if "tgp" in source or float(row.get("tgp_surface_prior", 0) or 0) >= 0.72:
        return "surface_charge_exploration"
    if pmpnn_norm >= 0.70:
...
def add_v61_brightness_teacher(df: pd.DataFrame, cfg: dict[str, Any], logger: Any | None = None) -> pd.DataFrame:
    out = df.copy()
    teacher_cfg = cfg.get("v61", {}).get("brightness_teacher", {})
    ...
    if not teacher_path or not Path(str(teacher_path)).exists():
        if logger:
            logger.info("V6.1 BRIGHTNESS TEACHER | no exact-sequence teacher CSV provided; using current model only")
    ...
    if logger:
        logger.info("V6.1 BRIGHTNESS TEACHER | file=%s | exact_sequence_coverage=%d/%d", teacher_path, int(out["brightness_teacher_coverage"].sum()), len(out))
    return out


def add_v61_physical_thermal_proxy(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
...
def add_v61_objective(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    out["v61_structure_score"] = (0.60 * safe_num(out, "structure_geometry_score", 0.05) + 0.40 * safe_num(out, "proteinmpnn_score_norm", 0.5)).clip(0.0, 1.0)
    out["v61_feedback_score"] = safe_num(out, "feedback_policy_score", 0.5).clip(0.0, 1.0)
    out["v61_blacklist_penalty"] = np.minimum(safe_num(out, "motif_blacklist_hits", 0.0), 4.0) / 4.0
    mut_count = safe_num(out, "mutation_count", 0.0)
    out["v61_mutation_burden_penalty"] = np.where(mut_count <= 5, 0.0, np.minimum((mut_count - 5.0) / 4.0, 1.0))
    out["v61_objective"] = (
        0.30 * out["brightness_lcb_norm"]
        + 0.20 * out["low_risk_score"]
        + 0.18 * safe_num(out, "thermal_proxy_v61", 0.5)
        + 0.12 * out["v61_structure_score"]
        + 0.08 * safe_num(out, "proteinmpnn_score_norm", 0.5)
        + 0.07 * out["v61_feedback_score"]
        + 0.05 * safe_num(out, "literature_score", 0.5)
        - 0.06 * safe_num(out, "epistasis_penalty", 0.0)
        - 0.05 * out["v61_mutation_burden_penalty"]
        - 0.04 * out["v61_blacklist_penalty"]
    ).clip(0.0, 1.0)
    # Compatibility aliases allow existing V6 notebooks and deploy scripts to keep working while ranking by V6.1 objective.
    out["v6_objective"] = out["v61_objective"]
    out["v5_objective"] = out["v61_objective"]
    return out
...
def select_portfolio(df: pd.DataFrame, cfg: dict[str, Any]) -> pd.DataFrame:
    ...
    hard_cfg = cfg.get("v61", {}).get("final_constraints", {})
...
def write_v5_reports(
    ...
    lines = [
        "# SynBio GFP V6.1 Adaptive Pipeline Report",
        "",
        "## Executive summary",
        "",
...
def write_submission(selected: pd.DataFrame, team_name: str, out_dir: Path) -> None:
    ...
    sub.to_csv(out_dir / "submission_v61.csv", index=False)
...
def write_v61_survival_reports(ranked: pd.DataFrame, out_dir: Path) -> None:
    rows = []
    for branch, sub in ranked.groupby(ranked.get("source_branch", ranked.get("source", pd.Series("unknown", index=ranked.index))).astype(str)):
        rows.append({
            ...
            "mean_v61_objective": float(safe_num(sub, "v61_objective", 0.0).mean()),
            "mean_thermal_proxy_v61": float(safe_num(sub, "thermal_proxy_v61", 0.0).mean()),
        })
    survival = pd.DataFrame(rows).sort_values(["final_eligible_rate", "mean_v61_objective"], ascending=False) if rows else pd.DataFrame()
    survival.to_csv(out_dir / "v61_feedback_survival_by_branch.csv", index=False)
    lines = ["# V6.1 Feedback Survival Audit", ""]
    if survival.empty:
    ...
    (out_dir / "v61_feedback_survival_report.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def run_pipeline_v6(cfg: dict[str, Any]) -> dict[str, str]:
    ...
    logger.info("V6.1 ADAPTIVE PIPELINE START")
    ...
    cand = add_v61_brightness_teacher(cand, cfg, logger)
    ...
    cand = add_v61_physical_thermal_proxy(cand)
    cand = add_v61_objective(cand)
    ...
    write_v61_survival_reports(ranked, out_dir)
    ...
    logger.info("V6.1 ADAPTIVE PIPELINE DONE | seconds=%.2f | final=%d", time.time() - start_time, len(selected))
    return {
        ...
        "final_top6": str(out_dir / "final_top6_v61.csv"),
        "submission": str(out_dir / "submission_v61.csv"),
