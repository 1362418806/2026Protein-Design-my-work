...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run complete SynBio GFP V6 traceable pipeline")
...
def select_portfolio(df: pd.DataFrame, cfg: dict[str, Any]) -> pd.DataFrame:
    ...
    min_hamming = int(final_cfg.get("min_pairwise_hamming", 2))
...
def write_v5_reports(
    ...
    lines = [
        "# SynBio GFP V6 Complete Pipeline Report",
        "",
        "## Executive summary",
        "",
...
def run_pipeline_v6(cfg: dict[str, Any]) -> dict[str, str]:
    ...
    logger.info("V6 COMPLETE PIPELINE START")
    ...
    logger.info("V6 COMPLETE PIPELINE DONE | seconds=%.2f | final=%d", time.time() - start_time, len(selected))
    return {
        ...
        "final_top6": str(out_dir / "final_top6_v6.csv"),
        "submission": str(out_dir / "submission_v6.csv"),
